<?php
require_once 'config.php';

// Only allow GET requests
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    sendError('Method not allowed', 405);
}

// Get and validate input parameters
$session_id = isset($_GET['session_id']) ? trim($_GET['session_id']) : '';

if (empty($session_id)) {
    sendError('Session ID is required', 400);
}

try {
    // Authentication and course rep verification is automatically handled by config.php
    // $user_id is available from global authentication in config.php

    // Get session details and verify ownership
    $session_sql = "SELECT s.*, c.course_name, c.course_code, d.group_name,
                           TIMESTAMPDIFF(MINUTE, s.session_start_time, s.session_end_time) as duration_minutes
                    FROM attendancesessions s
                    JOIN courses c ON s.course_id = c.course_id
                    JOIN departmentgroups d ON s.group_id = d.group_id
                    WHERE s.session_id = ? AND s.initiated_by_user_id = ?";
    
    $session_stmt = $conn->prepare($session_sql);
    if (!$session_stmt) {
        error_log("Session details prepare failed: " . $conn->error);
        sendError('Database error', 500);
    }
    
    $session_stmt->bind_param("si", $session_id, $user_id);
    $session_stmt->execute();
    $session_result = $session_stmt->get_result();
    
    if ($session_result->num_rows === 0) {
        sendError('Session not found or access denied', 404);
    }
    
    $session_info = $session_result->fetch_assoc();
    $session_stmt->close();

    // Get detailed attendance records for this session
    $attendance_sql = "SELECT ar.*, u.first_name, u.last_name, u.matric_number, 
                              u.profile_picture, u.department_id
                       FROM attendancerecords ar
                       JOIN users u ON ar.student_id = u.user_id
                       WHERE ar.session_id = ? 
                       ORDER BY ar.attendance_time ASC";
    
    $attendance_stmt = $conn->prepare($attendance_sql);
    if (!$attendance_stmt) {
        error_log("Attendance details prepare failed: " . $conn->error);
        sendError('Database error', 500);
    }
    
    $attendance_stmt->bind_param("s", $session_id);
    $attendance_stmt->execute();
    $attendance_result = $attendance_stmt->get_result();
    
    $attendance_records = [];
    while ($row = $attendance_result->fetch_assoc()) {
        $attendance_records[] = [
            'attendance_id' => (int)$row['attendance_id'],
            'student_id' => (int)$row['student_id'],
            'student_name' => trim($row['first_name'] . ' ' . $row['last_name']),
            'first_name' => $row['first_name'],
            'last_name' => $row['last_name'],
            'matric_number' => $row['matric_number'],
            'department_id' => (int)$row['department_id'],
            'attendance_time' => $row['attendance_time'],
            'selfie_url' => $row['selfie_image_path'] ?? null,
            'profile_picture' => $row['profile_picture'] ?? null,
            'attendance_type' => $row['attendance_type'] ?? 'present'
        ];
    }
    
    $attendance_stmt->close();

    // Calculate statistics
    $total_attendance = count($attendance_records);
    $attendance_by_hour = [];
    
    foreach ($attendance_records as $record) {
        $hour = date('H:00', strtotime($record['attendance_time']));
        if (!isset($attendance_by_hour[$hour])) {
            $attendance_by_hour[$hour] = 0;
        }
        $attendance_by_hour[$hour]++;
    }

    // Prepare comprehensive response data
    $response_data = [
        'session_details' => [
            'session_id' => $session_info['session_id'],
            'course_id' => (int)$session_info['course_id'],
            'group_id' => (int)$session_info['group_id'],
            'course_name' => $session_info['course_name'],
            'course_code' => $session_info['course_code'],
            'group_name' => $session_info['group_name'],
            'location' => $session_info['location'],
            'session_start_time' => $session_info['session_start_time'],
            'session_end_time' => $session_info['session_end_time'],
            'duration_minutes' => (int)$session_info['duration_minutes'],
            'ble_id' => $session_info['ble_id'],
            'initiated_by_user_id' => (int)$session_info['initiated_by_user_id'],
            'is_completed' => !is_null($session_info['session_end_time'])
        ],
        'attendance_records' => $attendance_records,
        'statistics' => [
            'total_attendance' => $total_attendance,
            'attendance_by_hour' => $attendance_by_hour,
            'first_attendance' => $total_attendance > 0 ? $attendance_records[0]['attendance_time'] : null,
            'last_attendance' => $total_attendance > 0 ? end($attendance_records)['attendance_time'] : null
        ],
        'retrieved_at' => date('Y-m-d H:i:s')
    ];
    
    sendSuccess('Session details retrieved successfully', $response_data);

} catch (Exception $e) {
    error_log("Get session details error: " . $e->getMessage());
    sendError('Failed to retrieve session details', 500);
}
?>
